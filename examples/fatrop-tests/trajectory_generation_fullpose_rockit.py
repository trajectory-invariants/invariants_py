...
optim_gen_results.invariants[:,:3], optim_gen_results.Obj_frames, optim_gen_results.FSr_frames, tot_time_rot = FS_online_generation_problem_rot.generate_trajectory(model_invariants[:,:3],boundary_constraints,new_stepsize,weight_params,initial_values)
...
while current_progress <= 1.0:
    ...
    boundary_constraints = {"orientation": {"initial": optim_iter_results.Obj_frames[current_index], "final": R_obj_end}, "moving-frame": {"initial": optim_iter_results.FSr_frames[current_index], "final": optim_iter_results.FSr_frames[-1]}}
    ...
    optim_iter_results.invariants[:,:3], optim_iter_results.Obj_frames, optim_iter_results.FSr_frames, tot_time_rot = FS_online_generation_problem_rot2.generate_trajectory(model_invariants[:,:3],boundary_constraints,new_stepsize,weight_params,initial_values)
...
boundary_constraints = {"orientation": {"initial": R_obj_start, "final": []}, "moving-frame": {"initial": FSr_start, "final": FSr_end}}
...
# for x in range(-2,3):
    ...
    optim_gen_results.invariants[:,:3], optim_gen_results.Obj_frames, optim_gen_results.FSr_frames, tot_time_rot = FS_online_generation_problem_rot3.generate_trajectory(model_invariants[:,:3],boundary_constraints,new_stepsize,weight_params,initial_values)
