...
# Solve
new_invars, new_trajectory, new_movingframes, tot_time_pos = FS_online_generation_problem.generate_trajectory(invariant_model = model_invariants, initial_values = initial_values, boundary_constraints = boundary_constraints, step_size = new_stepsize, weights_params = weights)
...
window_len = 20
# specify optimization problem symbolically
FS_online_generation_problem2 = FS_gen(boundary_constraints, window_len=window_len, fatrop_solver=use_fatrop_solver)
...
while current_progress <= 1.0:
    ...
    new_invars, calculate_trajectory, movingframes, tot_time_pos = FS_online_generation_problem2.generate_trajectory(invariant_model = model_invariants, initial_values=initial_values, boundary_constraints=boundary_constraints, step_size = new_stepsize, weights_params = weights)
    if use_fatrop_solver:
        print('')
        print("TOTAL time to generate new trajectory: ")
        print(str(tot_time_pos) + '[s]')
    ...
    # print(new_invars)
    # print(new_invars.shape)
    # print(calculate_trajectory.shape)
    # print(model_invariants.shape)
