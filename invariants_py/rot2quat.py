...
def rot2quat(R_all,single_rot=False):
    """
    ...
    if single_rot:
        R = orthonormalize(R_all)
        
        qs = np.sqrt(np.trace(R)+1)/2.0
        kx = (R[2,1] - R[1,2])/(4*qs)    # Oz - Ay
        ky = (R[0,2] - R[2,0])/(4*qs)    # Ax - Nz
        kz = (R[1,0] - R[0,1])/(4*qs)    # Ny - Ox
        
        q_all = np.array([kx,ky,kz,qs])
