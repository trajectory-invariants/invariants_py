...
class OCP_gen_pos:
    ...
    def generate_trajectory_global(self,U_demo,initial_values,boundary_constraints,step_size):
        ...
        # Initialize states
        for k in range(N):
            self.opti.set_initial(self.R_t[k], initial_values["moving-frame-position"][k])
        ...
        # Set values boundary constraints
        #self.opti.set_value(self.R_t_start,boundary_constraints["moving-frame-position"]["initial"])
        #self.opti.set_value(self.R_t_end,boundary_constraints["moving-frame-position"]["final"])
